...
type Repository interface {
	...
	FindOneWithOptions(filter primitive.M, options *options.FindOneOptions) *mongo.SingleResult
	...
	FindWithOption(filter primitive.M, option *options.FindOptions) (*mongo.Cursor, error)
	InsertMany(document []interface{}, insertOptions *options.InsertManyOptions) ([]interface{}, error)
}
...
func (m *MongoUserRepository) FindOneWithOptions(filter primitive.M, options *options.FindOneOptions) *mongo.SingleResult {
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()
	result := m.Collection.FindOne(ctx, filter, options)
	return result
}
...
func (m *MongoUserRepository) InsertOne(document interface{}) (interface{}, error) {
	...
	if err != nil {
		log.Printf("Error inserting document")
		return nil, errors.New("error inserting document")
	}
	return id.InsertedID, nil
}
func (m *MongoUserRepository) InsertMany(document []interface{}, insertOptions *options.InsertManyOptions) ([]interface{}, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()
	id, err := m.Collection.InsertMany(ctx, document, insertOptions)
	if err != nil {
		log.Printf("Error inserting document")
		return nil, errors.New("error inserting document")
	}
	return id.InsertedIDs, nil
}
...
func (m *MongoUserRepository) FindWithOption(filter primitive.M, option *options.FindOptions) (*mongo.Cursor, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()
	result, err := m.Collection.Find(ctx, filter, option)
	if err != nil {
		return nil, err
	}
	return result, nil
